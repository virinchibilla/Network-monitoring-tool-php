...
#Creating INFO table
$dbh->do("CREATE TABLE IF NOT EXISTS FRONTEND_vb (id INT AUTO_INCREMENT PRIMARY KEY,
										   IP varchar(255),
...
$dbh->do("INSERT INTO FRONTEND_vb (IP, PORT, COMMUNITY) SELECT DEVICES.IP, DEVICES.PORT, DEVICES.COMMUNITY from DEVICES ON DUPLICATE KEY UPDATE IP=FRONTEND_vb.IP")
		or die $DBI::errstr;
...
#RRD database
	foreach my $dev (keys %devices)
	{
		...
		{	
			...
			$dbh->do("UPDATE FRONTEND_vb SET webserver_time = '" . localtime() . "', Interface_List = '$if_mysql[0]', Interface_Name = '$if_mysql[1]'
...
#			my $if = oid_splitter($_);
		...
		{
			$dbh->do("UPDATE FRONTEND_vb SET $devices_ref->{$device}{$_}[0] = '$hash_ref->{$_}' 
