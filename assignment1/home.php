...
//Statement handle
$query = "SELECT IP, PORT, COMMUNITY, sysName, Interface_List, Interface_Name FROM FRONTEND_vb";
		...
		//		"--units-length", "5",
			...
			else
			{
				echo "<div class = 'col-md-6 col-centered' style = 'height: 100%; margin: 0 0 20px 0; text-align: center;'>
						...
						<form action = './page2.php' name = 'device' method = 'get'>
							<a href=./page2.php?device=" . $row["IP"] . "_" . $row["PORT"] . "_" . $row["COMMUNITY"] . "_" . $variable_line[$key] . '_' . $if_name[$key] . ">
								<img style = 'margin: 0;' src = './temp.php?location=" . $img_location . "' title = 'Interface " . $if_name[$key] . "'>
							</a>
						</form>
					  </div>";
			}
