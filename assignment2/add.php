...
//Drop plot_DEVICES_vb table
$query = "DROP TABLE IF EXISTS plot_DEVICES_vb";
mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
//Drop plot_SERVERS_vb table
$query = "DROP TABLE IF EXISTS plot_SERVERS_vb";
mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));


//Creating plot_DEVICES_vb table
$query = "CREATE TABLE plot_DEVICES_vb (id INT AUTO_INCREMENT PRIMARY KEY,
										   IP varchar(255),
...
//Creating plot_SERVERS_vb table
$query = "CREATE TABLE plot_SERVERS_vb (id INT AUTO_INCREMENT PRIMARY KEY,
										   IP varchar(255),
										   metrics LONGTEXT,
										   UNIQUE KEY (IP))";
mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
...
?>
					...
					<li role="presentation" class="active"><a href="./add.php">Select Devices/servers to plot</a></li>
...
//SERVERS
	...
	{
		$query = "INSERT INTO plot_SERVERS_vb (IP) VALUES ('" . $insert . "') ON
					DUPLICATE KEY UPDATE id = plot_SERVERS_vb.id";
		mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
	}
...
//DEVICES
	...
	foreach($_GET['if_list'] as $input 	=> $output)
	{
		...
		if($output == "all")
		{
			$query = "SELECT selected_list, selected_name from frontend_DEVICES_vb WHERE IP = '" . $exploding[0] . "' AND PORT = '" . $exploding[1] . "' AND COMMUNITY = '" . $exploding[2] . "'";
			$result = mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
	...
	foreach(array_keys($device) as $dev)
	{
		...
		$query = "INSERT INTO plot_DEVICES_vb (IP, PORT, COMMUNITY, Interface_List, Interface_Name) 
