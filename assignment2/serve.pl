...
$dbh->do("CREATE TABLE IF NOT EXISTS frontend_SERVERS_vb (id INT AUTO_INCREMENT PRIMARY KEY,
										   IP varchar(255),
										   UNIQUE KEY (IP))") or die $DBI::errstr;

#Inserting IP values frontend_SERVERS_vb into  table from Devices table
#$dbh->do("INSERT INTO frontend_SERVERS_vb (IP) SELECT SERVERS.IP from SERVERS ON DUPLICATE KEY UPDATE IP=frontend_SERVERS_vb.IP")
#		or die $DBI::errstr;

#Getting servers SERVERS_sai from database
$servers_data = $dbh->selectall_hashref("SELECT * from frontend_SERVERS_vb", 'id');
