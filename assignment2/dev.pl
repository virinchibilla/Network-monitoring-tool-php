...
#Creating INFO table
$dbh->do("CREATE TABLE IF NOT EXISTS frontend_DEVICES_vb (id INT AUTO_INCREMENT PRIMARY KEY,
										   IP varchar(255),
...
$device_data = $dbh->selectall_hashref("SELECT * from frontend_DEVICES_vb", 'id');
...
#RRD database
	...
	{	
		...
		$dbh->do("UPDATE frontend_DEVICES_vb SET webserver_time = '" . localtime() . "', Interface_List = '$if_mysql[0]', Interface_Name = '$if_mysql[1]'
...
sub save_oids()
{
	...
	elsif($identity eq 'bytes')
	{
		foreach(keys %$hash_ref)
		{
			...
			{
				$dbh->do("UPDATE frontend_DEVICES_vb SET $front_ref{$_} = '$hash_ref->{$_}' 
