...
?>
					...
					<li role="presentation"><a href="./add.php">Select Devices/servers to plot</a></li>
					<li role="presentation"><a href="./graph.php">Compare</a></li>
				</ul>
			</div><br>
...
{
	$query = "INSERT INTO frontend_DEVICES_vb (IP, PORT, COMMUNITY) VALUES ('" . $_GET['ip'] . "', '" . $_GET['port'] . "', '" . $_GET['community'] . "')
...
if(isset($_GET['button']) && !empty($_GET['if_list']))
{
	...
	foreach($_GET['if_list'] as $input => $output)
	{
		...
		if($output == "all")
		{
			$query = "SELECT Interface_List, Interface_Name from frontend_DEVICES_vb WHERE IP = '" . $exploding[0] . "' AND PORT = '" . $exploding[1] . "' AND COMMUNITY = '" . $exploding[2] . "'";
	...
	foreach(array_keys($device) as $dev)
	{
		...
		$query = "UPDATE frontend_DEVICES_vb SET selected_list = '" . $device[$dev]["iflist"]['string'] . "'" . ', selected_name = ' . "'" . $device[$dev]["ifname"]['string'] . "' 
...
//Table to select INTERFACES of a device to monitor from frontend_DEVICES_vb
$query = "SELECT * FROM frontend_DEVICES_vb";
$result = mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
...
if(isset($_GET['button']))
{
	if(!empty($_GET['device_monitor']))
	{
		foreach($_GET['device_monitor'] as $insert)
		{
			...
			$query = "DELETE FROM frontend_DEVICES_vb WHERE IP = '" . $exploding[0] . "' AND PORT = '" . $exploding[1] . "' AND COMMUNITY = '" . $exploding[2] . "'";
			mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
		}
...
{
	$query = "INSERT INTO frontend_SERVERS_vb (IP) VALUES ('" . $_GET['server_ip'] . "') ON DUPLICATE KEY UPDATE id=id";
	mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
	echo 'Server added';
}
...
if(isset($_GET['button']))
{
	if(!empty($_GET['server_monitor']))
	{
		...
		{
			$query = "DELETE FROM frontend_SERVERS_vb WHERE IP = '" . $insert . "'";
			mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
		}
...
//Table to Remove a server from frontend_SERVERS_vb
$query = "SELECT * FROM frontend_SERVERS_vb";
$result = mysqli_query($connection, $query) or die("Error:" . mysqli_error($connection));
